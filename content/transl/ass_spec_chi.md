...
- **[v4 Styles]**
  ...
  <span style='color: hibiscus'> ASS使用[v4 Styles+] </span>
...
|Script Type|这是SSA脚本格式版本，例如"V4.00"。如果您使用的SSA版本比创建脚本的版本旧的话，SSA将以此给出警告。<br /><span style='color: hibiscus'>ASS版本是"V4.00+"。</span>      |
...
|<span style='color: hibiscus'>WrapStyle</span>|<span style='color: hibiscus'>定义默认的换行样式。<br />0：智能换行，行均匀断开<br />1：行尾单词换行，只有\N断行<br />2：\N都断行<br />3：与0相同，但下行更宽。</span>|

## 5. 样式行，位于[v4<span display="inline-block" style='color: hibiscus'>+</span> Styles]部分
...
**Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, TertiaryColour, BackColour, Bold, Italic, <span display="inline-block" style='color: hibiscus'>Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle,</span> BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, AlphaLevel, Encoding**
...
|Fontsize| 字体大小。|
|PrimaryColour| 一个长整数BGR（蓝-绿-红）值。即此数字的十六进制等效中的字节顺序是BBGGRR，这是字幕通常显示的颜色。<span style='color: hibiscus'>ASS：</span>颜色格式也包含alpha通道。(AABBGGRR)</span>|
|SecondaryColour| 一个长整数BGR（蓝-绿-红）值。即此数字的十六进制等效中的字节顺序是BBGGRR，当字幕自动移动以防止画面碰撞时，可以使用此颜色代替主要颜色，以区分不同的字幕。<span style='color: hibiscus'>ASS：</span>颜色格式也包含alpha通道。(AABBGGRR)</span>|
|<span style='color: hibiscus'>OutlineColor</span> ~~(TertiaryColour)~~ | 一个长整数BGR（蓝-绿-红）值。即此数字的十六进制等效中的字节顺序是BBGGRR，当字幕自动移动以防止画面碰撞时，可以使用此颜色代替主要或次要颜色，以区分不同的字幕。<span style='color: hibiscus'>ASS：</span>颜色格式也包含alpha通道。(AABBGGRR)</span>|
|BackColour| 这是字幕轮廓或阴影的颜色，如果使用这些的话。一个长整数BGR（蓝-绿-红）值。即此数字的十六进制等效中的字节顺序是BBGGRR。<span style='color: hibiscus'>ASS：</span>颜色格式也包含alpha通道。(AABBGGRR)</span>|
...
|<span style='color: hibiscus'>Underline</span> | <span style='color: hibiscus'>下划线。[-1或0]</span>|
|<span style='color: hibiscus'>StrikeOut</span> | <span style='color: hibiscus'>删除线。[-1或0]</span>|
|<span style='color: hibiscus'>ScaleX</span> | <span style='color: hibiscus'>修改字体的宽度。[百分比]</span>|
|<span style='color: hibiscus'>ScaleY</span> | <span style='color: hibiscus'>修改字体的高度。[百分比]</span>|
|<span style='color: hibiscus'>Spacing</span> | <span style='color: hibiscus'>字符之间的额外空间。[像素]</span>|
|<span style='color: hibiscus'>Angle</span> | <span style='color: hibiscus'>旋转的原点由对齐方式定义。可以是浮点数。[度]</span>|
...
|Alignment| 这设置文本如何在屏幕左右边距内“对齐”，以及垂直放置。值可以是1=左，2=居中，3=右。为值加4表示"顶部标题"。为值加8表示"中间标题"。 例如5 = 左对齐的顶部标题 <span style='color: hibiscus'>ASS: Alignment，但按照数字键盘的布局（1-3底部，4-6中间，7-9顶部）。</span>|
...
|AlphaLevel| 这定义文本的透明度。SSA尚未使用此功能。<span style='color: hibiscus'>在ASS中不存在。</span>|
|Encoding| 这指定字体字符集或编码，在多语言Windows安装中，它提供对多种语言使用的字符的访问。对于英语（西方，ANSI）Windows，它通常为0（零）。<span style='color: hibiscus'>当文件是Unicode时，此字段在文件格式转换期间很有用。</span>|
...
|<span style='color: hibiscus'>Layer</span> | <span style='color: hibiscus'>（任何整数）具有不同layer编号的字幕在碰撞检测期间将被忽略。编号较高的layer将绘制在编号较低的layer之上。</span>|
...
|Effect|过渡效果。这为空，或包含SSA v4.x中实现的三种过渡效果之一的信息。<br />效果名称区分大小写，必须完全按照所示显示。效果名称周围没有引号。<br /><br />"Karaoke"意味着文本将一次一个单词地连续高亮显示。<span style='color: hibiscus'>Karaoke作为效果类型已过时</span>。<br /><br />"Scroll up;y1;y2;delay<span style='color: hibiscus'>[;fadeawayheight]</span>"意味着文本/图片将向上滚动屏幕。"Scroll up"一词后的参数用分号分隔。<br />y1和y2值定义屏幕上的垂直区域，文本将在其中滚动。值以像素为单位，哪个值（顶部或底部）在前并不重要。<br />如果值为零，则文本将在屏幕的整个高度上向上滚动。<br />delay值可以是1到100之间的数字，它会减慢滚动的速度——零意味着没有延迟，滚动将尽可能快。<br /><br />"Banner;delay"意味着文本将被强制为单行，无论长度如何，并从右到左在屏幕上滚动。<br />delay值可以是1到100之间的数字，它会减慢滚动的速度 - 零意味着没有延迟，滚动将尽可能快。<br /><br /><span style='color: hibiscus'>"Scroll down;y1;y2;delay[;fadeawayheight]"<br />"Banner;delay[;lefttoright;fadeawaywidth]"<br />lefttoright：0或1。此字段是可选的。默认值为0以使其向后兼容。当delay大于0时，移动一个像素将需要(1000/delay)秒。<br />（警告：Avery Lee的"subtitler"插件将"Scroll up"效果参数读取为delay;y1;y2）<br />fadeawayheight和fadeawaywidth参数可用于使侧边的滚动文本透明。</span>|
...
|\b<0或1>|\b1使文本变为粗体。\b0强制非粗体文本。<br />例如这里有一个{\b1}粗体{\b0}单词<br /><span style='color: hibiscus'>当此参数大于1时，它将用作字体的权重。（400 = 正常，700 = 粗体，注意：大多数字体将量化为2或3个厚度级别）</span>|
|\i<0或1>|\i1使文本变为斜体。\i0强制非斜体文本。<br />例如这里有一个{\i1}斜体{\i0}单词|
|\<span style='color: hibiscus'>\u<0或1></span>|\<span style='color: hibiscus'>下划线</span>|
|\<span style='color: hibiscus'>\s<0或1></span>|\<span style='color: hibiscus'>删除线</span>|
|\<span style='color: hibiscus'>\bord<宽度></span>|\<span style='color: hibiscus'>边框</span>|
|\<span style='color: hibiscus'>\shad<深度></span>|\<span style='color: hibiscus'>阴影</span>|
|\<span style='color: hibiscus'>\be<0或1></span>|\<span style='color: hibiscus'>模糊边缘</span>|
...
|\<span style='color: hibiscus'>\fsc<x或y><百分比></span>|<span style='color: hibiscus'><x或y>：x水平缩放，y垂直缩放 <百分比></span>|
|\<span style='color: hibiscus'>\fsp<像素></span>|<span style='color: hibiscus'><像素>：更改字母之间的距离。（默认：0）</span>|
|\<span style='color: hibiscus'>\fr[<x/y/z>]<角度></span>|<span style='color: hibiscus'><角度>：设置围绕x/y/z轴的旋转角度。<br />\fr默认为\frz。</span>|
...
|\a<对齐方式>|<对齐方式>是指定字幕屏幕对齐/定位的数字。<br />值1指定左对齐字幕<br />值2指定居中字幕<br />值3指定右对齐字幕<br />值加4指定"顶部标题"<br />值加8指定"中间标题"<br />0或不重置为样式默认值（通常为2）<br />例如{\a1}这是左对齐字幕<br />{\a2}这是居中字幕<br />{\a3}这是右对齐字幕<br />{\a5}这是左对齐的顶部标题<br />{\a11}这是右对齐的中间标题<br /><span style='color: hibiscus'>只有第一次出现才有效。</span>|
|\an<对齐方式>|数字键盘布局<br /><br /><span style='color: hibiscus'>只有第一次出现才有效。</span>|
|\k<持续时间>|<持续时间>是在具有Karaoke效果的对话事件中每部分文本高亮显示的时间量。持续时间以百分之一秒为单位。<br />例如{\k94}这{\k48}是{\k24}一个{\k150}卡拉OK{\k94}行<br /><br /><span style='color: hibiscus'>\k<持续时间>按单词高亮</span><br /><br /><span style='color: hibiscus'>\kf或\K<持续时间>从左到右填充</span><br /><br /><span style='color: hibiscus'>\ko<持续时间>从左到右轮廓高亮。</span>|
|<span style='color: hibiscus'>\q<数字></span>|<span style='color: hibiscus'><数字>换行样式</span>|
|\r<span style='color: hibiscus'>[<样式>]</span>|<样式>：这会取消一行中的所有先前样式覆盖<br /><span style='color: hibiscus'><样式>恢复为<样式>而不是对话行默认值。<br />任何样式修饰符后跟不可识别的参数都会重置为默认值。</span>|

### <span style='color: hibiscus'>函数</span>

|  |  |
|--|--|
|<span style='color: hibiscus'>\t([\<t1\>, \<t2\>, ] [<加速>,] <样式修饰符>)</span>|<span style='color: hibiscus'>\<t1\>, \<t2\>：动画开始，结束时间偏移[毫秒]（可选）<br /><加速>：修改变换的线性度（可选）<br />执行以下计算以获得在给定样式修饰符之间插值所需的系数：pow((t-t1)/(t2-t1), accel)，其中t是字幕的时间偏移。<br /><加速>的含义：<br />1：变换是线性的<br />在0和1之间：将开始快然后减慢<br />大于1：将开始慢然后变快<br />例如，使用2将使字母增长（通过{\fscx200\fscy200}）看起来是线性的，而不是减慢的。<br /><br /><样式修饰符>任何可以动画的样式修饰符：<br />\c,\1-4c,\alpha,\1-4a,\fs,\fr,\fscx,\fscy,\fsp,\bord,\shad,\clip（只有矩形\clip）</span> |
|<span style='color: hibiscus'>\move(\<x1\>, \<y1\>, \<x2\>, \<y2\>[, \<t1\>, \<t2\>])</span>|<span style='color: hibiscus'>\<x1\>, \<y1\>：开始的坐标。<br />\<x2\>, \<y2\> 结束的坐标。</span>|
|<span style='color: hibiscus'>\pos(\<x\>, \<y\>)</span>|<span style='color: hibiscus'>默认为\move(\<x\>, \<y\>, \<x\>, \<y\>, 0, 0)</span>|
|<span style='color: hibiscus'>\org(\<x\>, \<y\>)</span>|<span style='color: hibiscus'>将默认原点移动到(x,y)。这在沿旋转方向移动字幕时很有用。<br />警告：\t、\move和\pos将忽略碰撞检测。</span>|
|<span style='color: hibiscus'>\fade(\<a1\>, \<a2\>, \<a3\>, \<t1\>, \<t2\>, \<t3\>, \<t4\>)</span>|<span style='color: hibiscus'>\<a1\>：\<t1\>之前的Alpha值<br />\<a2\>：\<t2\>和\<t3\>之间的Alpha值<br />\<a3\>：\<t4\>之后的Alpha值<br />\<t1\>, \<t4\>：动画开始，结束时间偏移[毫秒]<br />\<t1\> - \<t2\>：Alpha值将在\<a1\>和\<a2\>之间插值<br />\<t2\> - \<t3\>：Alpha值将设置为\<a2\><br />\<t3\> - \<t4\>：Alpha值将在\<a2\>和\<a3\>之间插值<br /></span>|
|<span style='color: hibiscus'>\fad(\<t1\>, \<t2\>)</span>|<span style='color: hibiscus'>\<t1\>：淡入的时间长度<br />\<t2\>：淡出的时间长度</span>|
|<span style='color: hibiscus'>\clip(\<x1\>, \<y1\>, \<x2\>, \<y2\>)</span>|<span style='color: hibiscus'>剪裁参数定义的矩形之外的任何绘图。</span>|
|<span style='color: hibiscus'>\clip([<比例>,] <绘图命令>)</span>|<span style='color: hibiscus'>针对绘制的形状进行剪裁。<比例>与\p<比例>的情况具有相同的含义。</span>|

### <span style='color: hibiscus'>绘图</span>

|  |  |
|--|--|
|<span style='color: hibiscus'>\p<比例></span>|<span style='color: hibiscus'><比例>：打开绘图模式并同时设置坐标的放大级别。比例被解释为2的（<比例>减一）次方。例如{\p4}和坐标(8,16)将与{\p1}和(1,2)相同。此功能可用于亚像素精度。<br />如果为0，则关闭绘图模式，文本按通常方式解释。</span>|
|<span style='color: hibiscus'>\pbo<y></span>|<span style='color: hibiscus'><y>：基线偏移。默认情况下，任何绘图都定位在当前基线上。使用此值可以将它们向上或向下移动<y>像素。（向上：y<0，向下：y>0）</span>|

#### <span style='color: hibiscus'>绘图命令</span>

|  |  |
|--|--|
|<span style='color: hibiscus'>m \<x> \<y></span>|<span style='color: hibiscus'>将光标移动到\<x>, \<y></span>|
|<span style='color: hibiscus'>n \<x> \<y></span>|<span style='color: hibiscus'>将光标移动到\<x>, \<y>（未闭合的形状将保持开放）</span>|
|<span style='color: hibiscus'>l \<x> \<y></span>|<span style='color: hibiscus'>绘制一条到\<x>, \<y>的线</span>|
|<span style='color: hibiscus'>b \<x1> \<y1> \<x2> \<y2> \<x3> \<y3></span>|<span style='color: hibiscus'>使用点1和2作为控制点的三次贝塞尔曲线到点3</span>|
|<span style='color: hibiscus'>s \<x1> \<y1> \<x2> \<y2> \<x3> \<y3> .. \<xN> \<yN></span>|<span style='color: hibiscus'>三次均匀B样条曲线到点N，必须包含至少3个坐标</span>|
|<span style='color: hibiscus'>p \<x> \<y></span>|<span style='color: hibiscus'>将B样条曲线扩展到\<x>, \<y></span>|
|<span style='color: hibiscus'>c</span>|<span style='color: hibiscus'>关闭B样条曲线</span>|

### <span style='color: hibiscus'>您应该知道的事情：</span>

- <span style='color: hibiscus'>命令必须出现在{\p1+}之后和{\p0}之前。         （除了\clip(..)）</span>

- <span style='color: hibiscus'>绘图必须始终以移动到命令开始。</span>

- <span style='color: hibiscus'>绘图必须形成闭合形状。</span>

- <span style='color: hibiscus'>所有未闭合的形状将自动用直线闭合。</span>

- <span style='color: hibiscus'>对话行中的重叠形状将彼此进行XOR运算。</span>

- <span style='color: hibiscus'>如果相同的命令跟随另一个命令，则不需要再次写入其标识符字母，只需写入坐标。</span>

- <span style='color: hibiscus'>坐标相对于当前光标位置（基线）和对齐模式。</span>

- <span style='color: hibiscus'>命令p和c应该只跟随其他B样条命令。</span>

### <span style='color: hibiscus'>示例：</span>

- <span style='color: hibiscus'>正方形：m 0 0 l 100 0 100 100 0 100</span>

- <span style='color: hibiscus'>圆角正方形：m 0 0 s 100 0 100 100 0 100 c（在这种情况下，c等于"p 0 0 100 0 100 100"）</span>

- <span style='color: hibiscus'>圆形（几乎）：m 50 0 b 100 0 100 100 50 100 b 0 100 0 0 50 0（注意这里的第二个'b'是可选的）</span>
